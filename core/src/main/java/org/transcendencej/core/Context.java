...
import org.transcendencej.core.listeners.BlockChainListener;
import org.transcendencej.store.FlatDB;
import org.transcendencej.store.HashStore;
import org.transcendencej.store.MasternodeDB;
 ...
 * transcendencej for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating transcendencej {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use transcendencej (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a transcendencej context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using transcendencej!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing transcendencej via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.transcendencej.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.transcendencej.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.transcendencej.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    public void initSend(boolean liteMode, boolean allowInstantX) {
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (sporkManager!=null) {
            ...
            log.error("##### Send init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented send MN stuff
